...
```{r templateFigure1, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Neighborhoods in Berlin", fig.align='center'}
ggplot(raw) + 
...
```{r templateFigure2, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Number of Airbnbs by neighborhood in Berlin", fig.align='center'}
ggplot(sf_airbnb_by_neighborhood) +
...
```
```{r templateFigure3, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Proportion of Airbnbs by neighborhood in Berlin", fig.align='center'}
...
exception. We can use more detailed unit, called LOR (Lebensweltlich Orientierter Raum) - they are subdivisions, used by Berlin's municipal services for statistics and planning. Berlin provides a lot of data with that level of granularity.

```{r templateFigure4, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Number of Airbnbs by LOR", fig.align='center'}
...
This figure gives us much more detailed view - we can see that AirBnBs are highly  concentrated in the center, with the exception of Tiergarten-Brandenburger Tor area. There are not many residential buildings there, so it seems reasonable.
...
As we can see, the price distribution is not normal - it is skewed, with peak around 100 euros and long positive tail. This is usual for rent prices, as this category would include many different classes - both economy and luxury. We can also look if there is any variance in prices by neighborhood.

```{r templateFigure6, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Template", fig.align='center'}
ggplot(df_listings_cleaned, aes(x = neighbourhood_group_cleansed, y = price)) +
  ...
  labs(title = "Boxplot of Airbnb Prices by neighborhoods",
       ...
       y = "Price")
```
```{r templateFigure7, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Template", fig.align='center'}
ggplot(df_listings_cleaned, aes(x = neighbourhood_group_cleansed, y = price)) +
  geom_boxplot(fill = "lightblue") +
  coord_flip() +
  labs(title = "Boxplot of Airbnb Prices by neighborhoods",
...
```{r templateFigure8, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Template", fig.align='center'}
...
```{r templateFigure9, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Template", fig.align='center'}
