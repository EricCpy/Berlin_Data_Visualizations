...
nocite:
- "@corona_get_nodate"
- "@noauthor_lebensweltlich_2022"
- "@noauthor_gigabit_nodate"
- "@noauthor_strassenverkehrsunfalle_nodate"
- "@noauthor_standorte_nodate"
- "@noauthor_geodatensuche_nodate"
---
...
We also created an interactive map of our data, including train lines, which is useful for exploring specific Airbnb listings.
...
# Correlation between rent prices and number of Airbnbs
<h5 class="author">by Pavlo Kravets</h5>

In the recent years, Airbnb became controversial due to it's perceived impact on
prices and availability of long-term rentals. The idea is that short-term rentals
such as Airbnb price out the local population, as landlords find it more profitable to rent 
their properties to tourists. The truthfulness of that statement is beyond the
scope of this work. However, researching possible correlation between prices of 
long- and short-term rentals may be an interesting topic.

Sadly, obtaining the data on current long-term rent prices is not easy. This data
is not collected by the city of Berlin and specialized sites, such as Immoscout,
have implemented robust anti-scraping mechanisms. Still, we do have older data 
that details the offerings that were available on Immoscout in 2018. This work
assumes that the trends of high- and low-rent areas generally held true over time
and the data may be of some use.

```{r meanRentByLOR, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Mean rent by LOR", fig.align='center'}
ggplot(sf_rent_by_LOR) +
  geom_sf(aes(fill = mean_rent), color = "black", size = 0.2) +  # Smaller internal borders
  geom_sf(data = borough_borders, fill = NA, color = "black", size = 1) +  # Larger borders for main boroughs
  scale_fill_gradient(low = "lightyellow", high = "darkred", name = "Mean rent") +
  theme_void() +
  labs(fill = "Mean rent")

```

In case of long-term rentals it may make more sense to look int rents per square meter.

```{r meanRentPerSqmByLOR, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Mean rent per Sqm by LOR", fig.align='center'}
ggplot(sf_rent_by_LOR) +
  geom_sf(aes(fill = mean_rent_per_sqm), color = "black", size = 0.2) +  # Smaller internal borders
  geom_sf(data = borough_borders, fill = NA, color = "black", size = 1) +  # Larger borders for main boroughs
  scale_fill_gradient(low = "lightyellow", high = "darkred", name = "Mean rent per Sqm") +
  theme_void() +
  labs(fill = "Mean rent per Sqm")

```

We can see that there are several outlier LORs due to low number of rental offers
in them. Let's examine them, as well as the data without them.

```{r meanRentBoxplot, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Mean rent Outliers", fig.align='center'}
ggplot(sf_rent_by_LOR, aes(y = mean_rent)) +
  geom_boxplot(fill = "lightblue") +
  labs(y = "Mean rent price by LOR") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

```

As we see, there are some pretty extreme outliers in our data.
To obtain visualizations that provide some actual insight, we removed them.

```{r meanRentPerSqmByLORNoOutliers, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Mean rent per Sqm by LOR, outliers removed", fig.align='center'}
ggplot(sf_cleaned_rent_by_LOR) +
  geom_sf(aes(fill = mean_rent_per_sqm), color = "black", size = 0.2) +  # Smaller internal borders
  geom_sf(data = borough_borders, fill = NA, color = "black", size = 1) +  # Larger borders for main boroughs
  scale_fill_gradient(low = "lightyellow", high = "darkred", name = "Mean rent per Sqm") +
  theme_void() +
...
This map shows us similar a **circle** pattern around the center, 
but also other zones of high rents, such as south-west of Berlin.
Unlike Airbnb, long-term rentals ar more spread out. We may hypothesize that
locals are much more agreeable with living not in the center of the city.

```{r rentAirbnbScatter, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation between Rent and number of AirBnBs", fig.align='center'}
cor_spearman <- cor(rent_and_airbnb[["n"]], rent_and_airbnb[["mean_rent_per_sqm"]], method = "spearman", use = "complete.obs")
cor_pearson <- cor(rent_and_airbnb[["n"]], rent_and_airbnb[["mean_rent_per_sqm"]], method = "pearson", use = "complete.obs")

ggplot(rent_and_airbnb, aes(x = n, y = mean_rent_per_sqm)) + 
  geom_point(color = "blue", size = 1) +  
  geom_smooth(method = "loess", color = "red", se = FALSE) +
  theme_light() +                      
  labs(title = paste0("Pearson: ", round(cor_pearson, 2), " Spearman: ", round(cor_spearman, 2)),
       x = "Number of AirBnBs",               
       y = "Mean Rent per Sqm")               

```

As we can see, there is a strong positive correlation between the number of Airbnbs
and the rent, but it is hardly linear. Moreover, given our previous findings regarding
rents around the city, we cannot state with certainty that Airbnbs increase rent prices
in the area. However, they may gravitate towards more expensive parts of the city.
...
We want to estimate what influence the reputation of each *Bezirk* (district) might have on the price for a night.^[Probably the division by *Bezirk* is too rough. A division into *Kiez* (neighborhood) might be more promising to find some meaningful effects.] A first comparison of the median or mean prices per *Bezirk* suggests that there might be some meaningful differences to investigate on (see Table \@ref(tab:airbnb-price-by-bezirk-table)). Staying a night in *Mitte* seems to be twice as expensive as staying in *Reinickendorf*.
...
Figure \@ref(fig:airbnb-price-by-bezirk) shows the price for a night at Airbnbs per *Bezirk*. As we already pointed out, the prices have a long tail towards higher prices. For the 25% quantile of *Mitte* we find a value of `r q25_mitte` and for the 75% quantile of *Reinickendorf* a value of `r q75_reinickendorf`. Thus for these two extreme *Bezirke* the boxes show almost no overlap. When comparing any other pair, we find the boxes overlap more.

```{r airbnb-price-by-bezirk, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Showing the distribution of the price per Bezirk. Dashed line shows the 25% quantile of Mitte and the 75% quantile of Reinickendorf.", fig.align='center', fig.dim = c(8, 6)}
df_airbnb %>% ggplot(
  aes(
...
Figure \@ref(fig:airbnb-logprice-by-bezirk) shows the price for a night at Airbnbs per Bezirk on a logarithmic scale. For the remainder of this section we will use the logarithm of the Airbnb prices. Most of the price distributions look quite similar. Nevertheless, we will investigate whether we can find an effect on the Airbnb prices based on something we model as *Bezirk* reputation.

```{r airbnb-logprice-by-bezirk, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Showing the distribution of the logarithm of the price per Bezirk. Dashed line shows the 25% quantile of Mitte and the 75% quantile of Reinickendorf.", fig.align='center', fig.dim = c(8, 8)}
df_airbnb %>% ggplot(
  aes(
    y = neighbourhood_group_cleansed, 
    x = log_price, 
...
This section was written in parallel with the personal revision of Bayesian regression methods. Therefore, it includes discussions of DAGs and visualizations of simulated data, in addition to the real data from the Airbnb dataset. The first part of the chapter contains a step-by-step development of the initial model.
...
Following the @mcelreath_statistical_2020 book on Bayesian statistics, we start by specifying two base models and simulating data from them to compare it with the real data. Simulating data and comparing it to real data is a good way to get a sense of plausible priors [@mcelreath_statistical_2020, p. 85]. 

The most basic model assumes that the price follows a given distribution (e.g. normal distribution) and that there are no influences on price. This is probably not correct. A similar approach for simulation comes from a model where we believe that the price depends on some variables, but those variables were not measured. The corresponding DAG (directed acyclic graph) is shown in Figure \@ref(fig:dag-base-model).

```{r dag-base-model, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="DAG for the most basic model", fig.align='center', fig.dim = c(4, 4)}
dag <- dagify(
  price ~ u,
  latent = "u",
  outcome = "price",
...
As starting values for the base model a mean price of 100€ was chosen, assuming this might be a realistic price (before inspecting the real data). This corresponds to a value of `r log(100)` on the log scale. As standard deviation \(0.5\) was chosen. The statistical model is shown in Equation \@ref(eq:base-model). 
...
The statistical model formulated in Equation \@ref(eq:base-model) uses point-wise defined "priors" and is not a Bayesian but a frequentist model. In contrast, equation \@ref(eq:base-model-bayesian) specifies $\mu$ and $\sigma$ as distributions rather than single values. The mean and standard deviation chosen in the frequentist model are used as parameters in the prior for $\mu$, while $\sigma$ has a completely new distribution.
...
Since $\mu$ and $\sigma$ vary in the Bayesian model \@ref(eq:base-model-bayesian), we find that the simulated price range is wider compared to the simulations from the frequentist model \@ref(eq:base-model). The results can be seen in Figure \@ref(fig:price-distribution-simulated). By coincidence, the choice for the standard deviation produced a simulated distribution that is almost as wide as the real data for the Bayesian prior. For the frequentist prior the simulated distribution is even narrower, suggesting that the prior standard deviation might need to be slightly larger to be less restrictive.
...
Note, that the DAG shown in Figure \@ref(fig:dag-base-model) corresponds to both equations. The important role of DAGs is not to define an exact statistic model but to show the beliefs about causal effects that generate the real data. It is beneficial to create a graphical representation of the assumed causal model to select which covariables to include in the statistical model. In a DAG, one can identify for example, fork, pipe or collider constellations and detect backdoor paths. The decision on which covariable to include or exclude depends on the effect one is trying to estimate or the hypothesis being investigated.

Since the goal is to estimate the possible influence of the *Bezirk* reputation, it should be included in the DAG explicitly. Since there is no direct measurement of the *Bezirk* reputation (e.g., from a conducted survey), it is an unobserved (latent) variable. Because *Bezirk* reputation is the focus of our investigation, we will declare it as our exposure (treatment). Physically moving an Airbnb from one *Bezirk* to another is difficult.^[The *Bezirk* borders are unlikely to change in the near future, though they do rarely.] It might seem odd to consider the *Bezirk* reputation as the treatment. However, using the posterior the effect of changing the district can be calculated easily. Figure \@ref(fig:dag-reputation-only-model) shows the corresponding DAG.

```{r dag-reputation-only-model, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="DAG for model with reputation", fig.align='center', fig.dim = c(6, 4)}
dag <- dagify(
  price ~ u + reputation,
  latent = c("u"),
  exposure = "reputation",
...
A corresponding statistical representation can be seen in Equation \@ref(eq:reputation-model). The simulated distribution of log prices is shown in Figure \@ref(fig:price-distribution-simulated-reputation). The simulated distributions are spread out wider than for real data (see Figure \@ref(fig:airbnb-logprice-by-bezirk)), resulting in multiple pairs of districts where the boxes do not overlap. Thus, one can assume that the effect of *Bezirk* reputation is smaller than assumed in these priors.
...
A possible variable to include in the final model is the quality of the residential area (*Wohnlage*) [@noauthor_wohnlagen_2024].A high amount of good *Wohnlage* in a *Bezirk* might influence its reputation. It could also impact the rent or purchase price of a flat used as an Airbnb. Consequently, the host has to ask for a higher price per night to cover his rent or loan installment and might also seek to capitalize on the *Bezirk*'s good reputation to increase income further, thereby raising the price per night even more.

In Figure \@ref(fig:dag-reputation-model-wohnlage), one can see a model with *Wohnlage* added as a covariable. Since *Wohnlage* influences both reputation and price, it creates a fork pattern. *Wohnlage* is a classical confounder [@mcelreath_statistical_2020, p. 189].
...
Using a tool called Dagitty [@textor_robust_2016], one can draw DAGs directly in the browser. Furthermore, Dagitty provides a minimal adjustment set and information on which assumptions about conditional independence are testable if the specified model is correct. It is possible to indicate additional covariables one intends to use and check if they introduce any backdoor paths. The code for the DAG can also be used to display DAGs in R (e.g., with the **ggdag** package). Figure \@ref(fig:dag-reputation-model-wohnlage-adjustment-set) shows that *Wohnlage* is part of the so called minimal adjustment set. Thus, it must be included to estimate the (total or direct) causal effect.
...
One could also be tempted to include the results of the review sentiment analysis in the regression model. Assuming that reviews are influenced by people's expectations and their actual experience with an Airbnb visit, one can argue that reviews are affected by both price and *Bezirk* reputation. If the price is high expectations are likely high as well. If the reputation of the *Bezirk* is good, expectations might be high even if the price is modest. The resulting model can be seen in Figure \@ref(fig:dag-reputation-model-wohnlage-reviews).
...
Once again, the adjustment set includes only *Wohnlage*. Since *review* is influenced by both price and reputation, it acts as a collider. In contrast to a fork, a collider backdoor path is closed as long as the collider is not part of the model. Therefore, *review* should not be included in the model (see Figure \@ref(fig:dag-reputation-model-wohnlage-reviews-adjustment-set)).
...
If one wants to test whether an additional covariable would introduce backdoor paths, one can use the `ggdag_adjust()` function. Resulting backdoor paths are displayed as dashed lines. Figure \@ref(fig:dag-reputation-model-wohnlage-reviews-collider) shows that adjusting for reviews (including it in the model) introduces many non causal paths.^[I would recommend to use the Dagitty web app instead. There, paths affected by the collider are highlighted in red:<br>
...
The step-by-step model building description ends at this point. In the next section, the full model used for the regression analysis will be presented.

## Full model

The full model includes covariables of three types from various sources. The covariables framed by a green rectangle (see Figure \@ref(fig:dag-reputation-full-model)) are features provided by the Airbnb overview dataset. They yield basic information about how many people can stay, the number of rooms, and whether the Airbnb is an entire flat, a single room in a flat or a single room in a hotel.

The covariables framed by a purple rectangle come from various external data sources and were assigned to the flats based on their location. Most of this information is derived from datasets with geographical data aggregated by *LOR* and *Urnenwahlbezirk* or calculated distances to points of interest. We assume that the distance to train stations (especially Hauptbahnhof) might influence the price but not the reputation of a district. We also include the distance to public toilets, even though we don't expect a measurable influence on the price.

Lastly, we added more detailed information about the amenities in the Airbnbs, which we extracted from the full detailed Airbnb dataset.
...
It seems plausible that crime rates influence the reputation of a district. Additionally, theories predict higher crime rates in regions with lower living standards. This introduces a new backdoor path. However, it is already closed when the model is adjusted for *Wohnlage* (see Figure \@ref(fig:dag-reputation-full-model-minimal-adjustment-set)). Crime rate is not included in the final models because it may explain variance of *Bezirk* reputation, thereby reducing the covariance with price (precision parasite effect) and potentially amplifying the bias of unobserved confounders between exposure and target.^[Closing all backdoor paths is not the only criterion for selecting covariables. There are also other effects to consider.]
...
Indicating that all variables except reviews should be included in the model shows no non-closed backdoor paths (see Figure \@ref(fig:dag-reputation-full-model-adjustment-set)). Thus, the model might yield an unconfounded estimator for the effect of the *Bezirk* reputation. The reason for including additional covariables beyond those in the minimal adjustment set is that we aim to reduce the variance in Airbnb prices by reducing the amount of unobserved sources of variance. Hopefully this leads to narrower posterior predictions to identify a possible causal effect of the *Bezirk* reputation.
...
* distinguish between parameter estimations and predicted effects (by contrasting predictions).
* show the influence of robust regression assuming student distributed prices instead of normal distributed ones (not limited to Bayesian methods).

In Figure \@ref(fig:base-model-4x4), one can see the difference between the parameter estimation (distribution) of the mean price (left) and the posterior predictions for the distribution of Airbnb prices (right) for the model represented in Figure \@ref(fig:dag-base-model). It is clear that the distribution of the prices is much wider than the estimated distribution of the mean value because the result of the random sampling around the mean must be exponentiated.

Comparing the results from the model with $Normal(\mu, \sigma)$ distributed prices (top) with the results from the model that assumed $Student(2, \mu, \sigma)$ distributed prices (bottom), we observe a lower mean price for the student model. For the price distributions we find more Airbnb prices in the range of 200€ to 400€ for the normal model. In contrast, we find thicker tails for the student model, resulting in more predictions about Airbnb prices higher than 400€. The mean $\sigma$ value for the normal model is `r sigma_base_model` and for the student model `r sigma_base_model_student`. The maximum price for an Airbnb is `r max_pred_base`€ and respective `r format(max_pred_base_student, scientific=FALSE, big.mark="_")`€. For the student distribution we find more Airbnb prices predicted to be around 0€.
...
In Figure \@ref(fig:base-model-rep-wol-parameter), one can compare the parameter estimates for the normal and student distributed models once more. The DAG corresponding to the model these results are based on is shown in Figure \@ref(fig:dag-reputation-model-wohnlage). The means are estimated for each *Bezirk* and according to each value of *Wohnlage*. We can see that the means for the highest value of *Wohnlage* are separated from those of the lower two values. For the latter, there is substantial overlap of the distributions. The separation appears somewhat better for the student model. It is also clear that some parameter distributions show no overlap comparing the same *Wohnlage* among districts.
...
To estimate the causal effect of the *Bezirk* reputation, it is necessary to inspect contrast plots (see Figure \@ref(fig:base-model-rep-wol-parameter-contrasts)). To limit the amount of comparisons the districts *Mitte* (high prices), *Neukölln* and *Tempelhof-Schöneberg* (medium prices) and *Reinickendorf* (low prices) have been selected. It can be seen that the mean price (aggregated over all *Wohnlagen*^[To be precise, one would have to select prices according to the distribution of the *Wohnlage* values per district. It was assumed that they all make up one third in each district to simplify the analysis.]) differ by more than 10€ for all comparisons except for the pair *Neukölln* and *Tempelhof*. One could conclude that moving an Airbnb from *Reinickendorf* to *Mitte* allows for a price increase of around 50€.
...
Next we will examine how the difference in mean price relates to the prediction of the Airbnb price distribution (see Figure \@ref(fig:base-model-rep-wol-predictions)). Here the price distributions for the normal and student model look quite similar. The thicker tails of the student model are barely visible. However, the maximum predicted price is, once again, much too high and there are many predictions around 0€. Therefore, we will not use the student modeling approach for the full model later.

The price distributions for all values of *Wohnlage* almost completely overlap, even though the parameter estimations for their means were distinguishable. Aside from the distributions for *Reinickendorf* and *Marzahn-Hellersdorf* (which are a little bit more left skewed), they all look pretty similar in this plot.
...
To investigate the difference of the predicted price distributions, we create a contrast plot (see Figure \@ref(fig:base-model-rep-wol-predictions-contrasts)). It can be seen that the distributions of price differences have an considerable amount of mass on both sides of zero. Nevertheless, the probability of getting a lower price for an Airbnb in *Reinickendorf* compared to *Mitte* is predicted to be 75.6 %. The probability of saving more than 10€ per night is still 70.4%. Even though it was difficult to see any difference without the contrast plot and with zero close to the peak of probability distributions, we find results that are meaningful in reality.
...
* There is an important difference between parameter distributions and prediction distributions. The prediction distributions are wider. This is not a drawback but rather an honest representation of uncertainty.  It is crucial not to report only the parameter distributions. Uncertainty should not be hidden but rather embraced to make thoughtful decisions.
* The causal effect has to be inspected via contrasts, not by overlaying distributions. Often, low values in one distribution correspond to low values in the other, making the difference consistent.
* Although the student distribution should yield models that are more robust to outliers, it also generates more outlier predictions. This approach does not necessarily narrow the distributions.^[We were tempted to use a normal distribution for predictions with the $\sigma$ from the student analysis because this would make the distributions narrower, giving the appearance of greater certainty. But to my current understanding this might be cheating towards nicer results.]

### Full model

The full models DAG was already presented in Figure \@ref(fig:dag-reputation-full-model). For the full model all additional covariables were either included as numeric predictors and scaled before or treated as binomial. The only covariable modeled as an ordered predictor is *Wohnlage*. Treating covariables such as the number of rooms as numeric predictors was simpler. In a real analysis, one might model variables like the model number of bathrooms as ordered predictors rather than numeric ones, as it is more significant to distinguish between having zero or one bathroom compared to having two or three. Additionally, it does not make sense to predict negative room numbers or fractions on a finer granularity than a half.

The estimations and predictions in the next figures are based on the average values for numeric predictors (e.g., an average number of beds for an average number of accommodations) for Airbnbs with minimal amenities. This means no WiFi, TV, sauna or kitchen at all. The next section will discuss whether the parameter estimations from our model can be interpreted as causal effects and thus contrast plots for these covariables might be of interest as well.

In Figure \@ref(fig:full-model-parameter), the parameter estimations for Airbnbs in different districts grouped by *Wohnlage* are shown and this time also grouped by property type. As before, the estimation for a *Wohnlage* value of three is separated from the other two levels. However, the parameter distributions appear to be distinguishable especially by the type of property. Shared rooms are less expensive, while hotel rooms are the most costly. This is may be the key factor that made Airbnb big in the first place. One can get a whole flat for less than a single hotel room. One can see that the estimation for hotel rooms are pretty wide. This reflects the uncertainty related to the low number of data points for hotel rooms in the Airbnb dataset.
...
Since even the price distributions for mean prices spread out above 200€, it becomes challenging to get any idea about differences caused by *Bezirk* reputation. In Figure \@ref(fig:full-model-parameter-contrasts), we see the contrasts aggregated over all levels of *Wohnlage* and property types. In this case none of the comparisons of the estimated mean prices is free of overlap. Interestingly, it seems like *Neukölln* is now more similar to *Mitte* than to *Tempelhof-Schöneberg*. In fact *Neukölln* seems to be even more expensive than *Mitte*. A possible explanation for this could be the amenities. If the Airbnbs in *Mitte* offer better amenities than those in *Neukölln* this can explain the higher prices found at the first inspection of the data.
...
In Figure \@ref(fig:full-model-parameter-roomtype-contrasts), the mean price distributions are inspected separately by property type. It is evident that a clear distinction in mean price is found for entire homes/apartments, as well as for private and hotel rooms. In contrast, the difference for a shared room is much smaller among the districts.
...
Finally we will show the results for the posterior predictions for the actual Airbnb prices. Figure \@ref(fig:full-model-pred) shows that there is substantial overlap among all distributions, as already observed before. Nevertheless, the order of prices, which may be related to property type, can still be distinguished.
...
From the contrasts plot aggregated over *Wohnlage* and property type (see Figure \@ref(fig:full-model-pred-contrasts)), one can see that the probability of finding a cheaper Airbnb (with equal amenities and size) decreased from 73 % to 65 % for the pair *Mitte*-*Reinickendorf*, but increased slightly from 66 % to 67 % for the pair *Neukölln*-*Reinickendorf*. In general, the probability of finding a difference approached 50%. Thus the causal effect of *Bezirk* reputation seems to be smaller than estimated with the model that only accounted for the minimal adjustment set. This seems plausible since we possibly compared luxurious Airbnb against simpler ones before. Now the model can account for these sources of variance in the target variable.
...
Inspecting the posterior predictions separated by property type (see Figure \@ref(fig:full-model-pred-roomtype-contrasts)) does not reveal any new interesting patterns. This was different when the mean price distributions were inspected.
...
In Figure \@ref(fig:comparing-median-prices), one can see the median prices for Airbnbs as found in the real dataset and predicted from the full model. The differences can likely be explained by the fact that the model predictions assume Airbnbs of equal size with the same amenities. The difference between the predicted median prices from the full model can be interpreted as reflecting the effect of *Bezirks* reputation. The fact that the median price for Airbnbs in *Spandau* is the highest was somewhat surprising. Our personal expectations were:
...
Regarding the prediction for *Spandau*, it is worth revisiting Table \@ref(tab:airbnb-price-by-bezirk-table) and Figure \@ref(fig:base-model-rep-wol-parameter). Since the dataset has very few information about Airbnbs in *Spandau* (as for *Lichtenberg* and *Marzahn-Hellersdorf*) it is pretty uncertain about the corresponding parameter.^[Using a hierarchical model could help to prevent possible overfitting for districts with less information by learning a common distribution shape across all districts. Unfortunately we had not enough time to review this topic in depth yet.] This uncertainty is not reflected in Figure \@ref(fig:comparing-median-prices). In fact this is a crucial point Bayesian statisticians emphasizes often: **The result is the whole distribution. At least the point estimator and the width of an credibility interval.** Presenting only a point estimate can be misleading when making decisions. However, simple point estimates seem to be the kind of answers people want to get. But life is not simple, it is complex and full of uncertainty.
...
To be fair, communicating uncertainty alongside displaying shifts in rankings is challenging. In Figure \@ref(fig:comparing-median-prices-uncertainty-circles) the uncertainty is reflected by the diameter of the added circles. For the raw data the number of data points per *Bezirk* has been normalized and multiplied by `-1`. Thus a small diameter represents a higher count of data for a given *Bezirk*. For the predictions from the full model the standard deviations of the predicted prices grouped by *Bezirk* have been scaled. Therefore, a big diameter represents a wide distribution for the predictions. The diameter sizes should not be compared across the two columns, they are only representative within each column.

This approach may not be as clear as one might wish because the size of circles is hard to distinguish and the circles tend to overlap. However, this approach seems to be better than comparing half-eye distributions or boxplot, as it allows for a clearer identification of shifts in median prices. Combining two half-eye plots with a line plot can also clarify the reordering of rankings (see \@ref(fig:comparing-median-prices-half-eye)). This might be more suitable when the shape of the predicted distributions become more variable.
...
For *Wohnlage* the estimated parameter only can be interpreted as the direct effect but not as the total causal effect. Same holds for:
...
The mean of the parameter estimates, as well as indicators of their distribution widths can be found in \@ref(tab:full-model-parameter-summary). This table also specifies whether the parameter can be interpreted as total and direct causal effect or only as direct causal effect. The table is linked with Figure \@ref(fig:full-model-contrasts-interactive), which shows the contrast plots for the selected covariable. On the left it shows the effect on the mean price and on the right it shows the effect for the predicted actual prices.^[It might be necessary to zoom a little bit out to see all three depending on the size and resolution of the screen. Since the interactive Plotly apps produce a lot of data that has to be stored and loaded in the HTML document the densities are generated by `r ndraw` data points per parameter only. Later we used a ShinyApp where the densities are based on 1000 data rows. The percentages are computed on all data points and thus more precise.]

The model is often quite certain that the mean price for an Airbnb with a selected amenity (or with an increased value by one standard deviation for a numeric predictor) is higher (or lower). Nevertheless, the model is much less certain that prices actually found in reality will be higher (or lower) for most Airbnbs. For example, the model is 100% certain that an Airbnb with balcony is in general (mean) more expensive than one without. But it predicts only a 55% probability that for a given pair of Airbnbs the one with a a balcony is more expensive than one without. For an Airbnb where a number of accommodates is one standard deviation higher than for the population mean the model is 100% certain that the mean price is higher. For an actual pair it predicts 64% probability that the Airbnb with space for more accommodations is more expensive.
...
```{r violinPlotCITitles, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Prices (95% interval) of title sentiments", fig.align='center', fig.width=10, fig.height=4}
...
Our hypothesis is that Germans often come to Berlin to visit friends and family, while English speaking visitors are more likely to be tourists.
...
In conclusion, our analysis of Airbnb reviews and titles in English and German reveals that positive sentiment marginally increase listing prices, its impact is relatively minor compared to other factors such as images and amenities. Reviews tend to be mostly positive or neutral, with negative reviews often linked to higher priced listings, which is potentially due to higher expectations or selection bias. Sentiment also varies across districts, with central areas showing more negative reviews. Additionally, German reviews show more extreme sentiments, both positive and negative, compared to the generally neutral tone of English reviews. These findings underscore the influence of sentiment in Airbnb reviews and titles.

From the regression analysis, we found that the ranking in median prices for Airbnbs changes when the characteristics, amenities and type as well as the quality of its neighborhood of an Airbnb is taken account for in a model. Representing the certainty of predictions in every visualization can be challenging. Sometimes, it is more appropriate to have multiple visualizations that focus on different aspects side by side. None of the modeled causal effects alone leads to distinguishable distributions for price predictions but for the mean prices they do. Adding up multiple effects leads to noticeable shift. Figure \@ref(fig:multiple-contrasts) shows the contrast plot for two hypothetical Airbnbs that differ in half of their price influencing characteristics and amenities. In this case, the model is quite confident about the outcome.

```{r multiple-contrasts, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Comparing predictions for actual prices for Airbnbs that differ in half of the price influencing parameters.", fig.align='center', fig.dim = c(4, 3)}
