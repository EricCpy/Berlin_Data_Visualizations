...
max_pred_base <- 1
max_pred_base_student <- 1
sigma_base_model <- 1
sigma_base_model_student <- 1
# sigma_base_model <- summary(base_model_student)[1,1]
# sigma_base_model_student <- summary(base_model_student)[2,1]
# max_pred_base <- base_model %>% spread_draws(mu, sigma, ndraws = 1000) %>% mutate(price = exp(rnorm(1000, mu, sigma))) %>% pull(price) %>% max()
# max_pred_base_student <- base_model_student %>% spread_draws(mu, sigma, ndraws = 1000) %>% mutate(price = exp(rstudent(1000, 2, mu, sigma))) %>% pull(price) %>% max()
...
```{r base-model-rep-wol-parameter-contrasts, echo=FALSE, message=FALSE, warning=FALSE, cache=full_refit, fig.cap="Bla.", fig.align='center', fig.dim = c(8, 8)}
...
```{r base-model-rep-wol-predictions-contrasts, echo=FALSE, message=FALSE, warning=FALSE, cache=full_refit, fig.cap="Contrast plot of price difference among four districts. Positive values: one safes money if one books an Airbnb in the *Bezirk* named right of the minus sign.", fig.align='center', fig.dim = c(8, 8)}
bdat <- base_model_reputation_wohnlage_post %>% 
  pivot_wider(names_from = "wohnlage", values_from = "delta", names_prefix = "wol_") %>%
  rowwise() %>% 
  mutate(
...
With minimal adjustment set: Wohnlage total effect yes, direct effect not
With chosen set: both
Sauna yes

should crime rate be included? reduces variance of reputation? can have parasite effect
...
```{r neighborhoodSentiments, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Review sentiments of neighborhoods", fig.align='center'}
...
```{r neighborhoodAirbnbCount, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Number of Airbnbs of neighborhoods", fig.align='center'}
...
```{r correlationCountSentiment, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation Sentiment and Number of Airbnbs of neighborhoods", fig.align='center'}
...
```{r correlationPriceSentiment, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation sentiment and price and rating and price", fig.align='center'}
...
```{r correlationPriceSentimentRatingNeighborhoods, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation sentiment and price and rating and price", fig.align='center'}
ggplot(df_listings_cleaned_with_review_sentiment, aes(x = review_scores_rating, y = price)) +
...
ggplot(df_listings_cleaned_with_review_sentiment, aes(x =mean_sentiment_score_for_reviews , y = price)) +
  geom_point(aes(color = neighbourhood_group_cleansed), alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE, aes(color = neighbourhood_group_cleansed)) +
  facet_wrap(~ neighbourhood_group_cleansed) + 
  labs(
...
```{r yearSentiments, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Review sentiments of neighborhoods", fig.align='center'}
...
```{r monthSentiments, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Review sentiments of neighborhoods", fig.align='center'}
