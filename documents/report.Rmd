...
**TODO show the price distribution (normal distribution)**

## Data description

Two types of datasets were used. First a dataset providing information about 8384
Airbnb Listings in Berlin. It includes various attributes related to each listing,
such as the listing ID, name of the listing, information about the host, the 
neighborhood, the room type, the price, and the availability throughout the year.
Then datasets providing collections of reviews written in different languages for
Airbnb listings in Berlin. It includes review, reviewer and listing IDs, as well
as a date and the textual content of the comment of the review.
...
ggplot(raw) + 
  ...
  theme_void() +
  labs(title = "Neighborhoods in Berlin",
       fill = "Bezirk")
```
...
ggplot(raw) +
  ...
  theme_void() +
  labs(title = "Number of Airbnbs by neighborhood in Berlin",
...
ggplot(airbnb_count, aes(x = reorder(BEZ_NAME, proportion), y = proportion)) +
  ...
  coord_flip() +
  labs(title = "Proportion of Airbnbs by neighborhood in Berlin",
       x = "Bezirk",
...
By plotting maps of the number or the proportion of airbnbs in the different
neighborhoods of Berlin, we can see that most of the airbnbs are located in the 
central neighborhoods of Berlin, with Tempelhof-Schoneberg being the only 
exception. Thus the neighborhoods on the outside of Berlin contain a smaller 
amount of airbnb listings. 

```{r templateFigure, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Template", fig.align='center'}
ggplot(df_listings_cleaned, aes(x = neighbourhood_group_cleansed, y = price)) +
  geom_boxplot(fill = "lightblue") +
  coord_flip() +
  labs(title = "Boxplot of Airbnb Prices by neighborhoods",
       x = "Bezirk",
       y = "Price")
```
```{r templateFigure, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Template", fig.align='center'}
...
We can also try to compare the neighborhoods on different criteria, such as the
price or the availability of the listings. As for the prices it is difficult to
define clear tendencies, but as an example, listings in Mitte have a higher
median price.
...
```
Finally we can look at the different types of rooms in the listings. And we can
see that most of them are entire homes/appartements, and only some of them are private
rooms.
...
```{r comparing-median-prices, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Comparing the median prices for Airbnbs found in the raw data with predictions from the full model.", fig.align='center', fig.dim = c(6, 6)}
...
gg_pred <- shared_trt2 %>% 
  ggplot(aes(x = price_diff, group = parameter)) +
  geom_density(fill = "skyblue") +
  geom_text(aes(x = 200, 0.005, label = perc, group = parameter), hjust=0.5, vjust=0.5) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(x="difference of mean price in €", y=NULL) +
  theme(
    legend.position = "bottom"
...
<p align="center">
  <img src="images/languages.png" alt="Top 10 languages by review count">
</p>

Due to the computational effort required to analyze reviews in all languages (estimated 30 hours of computation time), we decided to analyze only the English and German texts. We also considered translating all texts into English after detecting the language using translation libraries that access APIs like Google Translate, as well as translation models. However, this approach also posed computational challenges (estimated > 10 hours). Consequently, we chose to focus solely on the English and German texts.

For sentiment analysis, we used the [VADER Sentiment Analysis tool](https://github.com/cjhutto/vaderSentiment) for English texts, which is fast but less robust. For German texts, we used the [German Sentiment tool](https://github.com/oliverguhr/german-sentiment), which is slower but more robust.
...
<p align="center">
  <img src="images/wordcloud_titles.png" alt="Wordcloud of Titles" width="85%">
</p>
...
```{r sentimentsTitles, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Sentiment count of titles", fig.align='center', fig.width=5, fig.height=4}
...
Here are some examples of negatively classified titles:
...
```{r violinPlotLogTitles, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Prices (log) of title sentiments", fig.align='center', fig.width=5, fig.height=4}
...
```{r violinPlotCITitles, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Prices (95% CI) of title sentiments", fig.align='center', fig.width=8, fig.height=4}
...
```{r sentimentsOfreviews, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Sentiment count of reviews", fig.align='center', fig.width=5, fig.height=4}
...
```{r violinPlotsReviewPrices, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Prices of review sentiments", fig.align='center', fig.width=8, fig.height=4}
mean_price <- mean(df_listings_cleaned$price)
# mean_price
sd_price <- sd(df_listings_cleaned$price)
# sd_price
...
### Sentiment and rating distribution across neighborhoods
...
We can observe that the eastern and western outer districts of Berlin have more positive sentiments in their reviews compared to the central districts. We saw in our overview chapter that the central districts have a higher number of Airbnbs and a higher price. Almost every district with a large number of Airbnbs and high prices tends to have a negative sentiment, with the exception of Neukölln. Neukölln is unique in that it has a negative sentiment despite having fewer Airbnb listings and lower prices.

#### Sentiment compared to number of reviews
The results from the previous chapter raised the question of whether our assumption holds true, that sentiment might be affected by the number of Airbnbs in a district. To explore this, we have plotted the number of Airbnbs per district again, as we did in the previous chapter with the sentiments.

```{r neighborhoodAirbnbCount, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Number of Airbnbs of neighborhoods", fig.align='center', fig.width=8, fig.height=4}
...
When comparing this chart with the sentiment one, it is immediately apparent that there is a strong correlation between the number of Airbnbs and the sentiment. The following plot illustrates this relationship more clearly.

```{r correlationCountSentiment, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation Sentiment and Number of Airbnbs of neighborhoods", fig.align='center', fig.width=5, fig.height=4}
...
```{r correlationPriceSentiment, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation sentiment and price and rating and price", fig.align='center', fig.width=8, fig.height=4}
...
```{r correlationPriceSentimentRatingNeighborhoods, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation sentiment and price and rating and price", fig.align='center', fig.width=8, fig.height=4}
rating_plot <- ggplot(df_listings_cleaned_with_review_sentiment, aes(x = review_scores_rating, y = price)) +
  ...
  labs(
    title = "Price vs Rating score by neighborhood",
    x = "Rating",
    y = "Price"
  ) +
  theme_minimal() +
...
sentiment_plot <- ggplot(df_listings_cleaned_with_review_sentiment, aes(x =mean_sentiment_score_for_reviews , y = price)) +
  ...
  facet_wrap(~ neighbourhood_group_cleansed) + 
  labs(
    title = "Price vs review sentiment score by neighborhood",
    x = "Review Sentiment Score",
    y = "Price"
  ) +
  theme_minimal() +
...
```{r anovaTest, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Correlation sentiment and price and rating and price", fig.align='center'}
...
```{r yearSentiments, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Review sentiments of neighborhoods", fig.align='center', fig.width=8, fig.height=4}
...
```{r monthSentiments, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Review sentiments of neighborhoods", fig.align='center', fig.width=8, fig.height=4}
...
Overall, it appears that spring is a season with notably high sentiment. In contrast, summer and winter seem to be less favored. This could be because additional amenities, such as air conditioning or heating, are required during these seasons but not necessarily during spring.  
...
```{r distributionOfLanguagesOverneighborhoods, echo=FALSE, message=FALSE, warning=FALSE, cache=!full_refit, fig.cap="Sentiment count of reviews", fig.align='center', fig.width=8, fig.height=4}
plot_ger <- ggplot() +
  geom_sf(data = lor_combined, aes(geometry = geometry, fill = label_combined)) +
  geom_sf_text(data = lor_combined, aes(geometry = geometry, label = BEZ_ID), size = 3, color = "black") +
